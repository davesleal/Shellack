...
class TestTurnCost:
    def test_turn_cost_calculation_haiku(self):
        turn = TurnCost(input_tokens=1000, output_tokens=500, model="claude-haiku-4-5-20251001")
        cost = turn.calculate()
    ...
    def test_turn_cost_calculation_sonnet(self):
        turn = TurnCost(input_tokens=2000, output_tokens=1000, model="claude-sonnet-4-6")
        turn.calculate()
    ...
    def test_turn_cost_unknown_model_uses_default(self):
        turn = TurnCost(input_tokens=1000, output_tokens=1000, model="claude-unknown-99")
        turn.calculate()
...
class TestThreadCost:
    def test_thread_cost_accumulates(self):
        thread = ThreadCost()
        thread.add_turn(TurnCost(input_tokens=1000, output_tokens=500, model="claude-sonnet-4-6"))
        thread.add_turn(TurnCost(input_tokens=2000, output_tokens=1000, model="claude-sonnet-4-6"))
    ...
    def test_total_tokens(self):
        thread = ThreadCost()
        thread.add_turn(TurnCost(input_tokens=100, output_tokens=50, model="claude-sonnet-4-6"))
        thread.add_turn(TurnCost(input_tokens=200, output_tokens=75, model="claude-sonnet-4-6"))
