...
def test_run_session_close_records_session():
    ...
    ), patch("bot_unified.get_channel_name", return_value="alpha-dev"), patch.object(
        bot_unified.usage_tracker, "record_session"
    ) as mock_record, patch.dict(
...
def test_project_message_records_mention():
    ...
    fake_routing = {
        "alpha-dev": {"mode": "dedicated", "project": "alpha", "channel_id": "C_ALPHA"}
    }
    fake_projects = {"alpha": {"name": "Alpha", "path": "/tmp", "language": "python"}}

    with patch("bot_unified.agent_factory") as mock_factory, patch(
        "bot_unified.app", mock_app
    ), patch("bot_unified.CHANNEL_ROUTING", fake_routing), patch(
        "bot_unified.PROJECTS", fake_projects
    ), patch(
        "bot_unified.ThinkingIndicator"
    ) as mock_indicator_cls, patch.object(
        bot_unified.usage_tracker, "record_mention"
    ) as mock_record, patch.dict(
        "os.environ", {"SESSION_BACKEND": "api", "SESSION_MODEL": "claude-sonnet-4-6"}
    ):
