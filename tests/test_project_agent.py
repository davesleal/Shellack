...
def make_agent(tmp_path):
    ...
    with patch("agents.project_agent.GitHubClient"), \
         patch("peer_review.StagedPeerReview"), \
...
def test_handle_calls_lifecycle_started(tmp_path):
    ...
    with patch.object(agent._lifecycle, "started") as mock_started, \
         patch.object(agent, "_trigger_peer_review"):
...
def test_handle_triggers_review_when_response_has_code_block(tmp_path):
    """General response with ``` triggers peer review."""
    agent, app, client = make_agent(tmp_path)
    client.messages.create = MagicMock(
        return_value=MagicMock(content=[MagicMock(text="Here:\n```swift\nlet x = 1\n```")])
    )
    with patch.object(agent, "_trigger_peer_review") as mock_review, \
         patch.object(agent, "_write_journal"):
