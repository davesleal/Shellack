...
def test_plugins_command_calls_list_all():
    ...
    with patch.object(
        bot_unified.plugin_manager, "list_all", return_value=list_all_result
    ) as mock_list, patch("bot_unified.get_channel_name", return_value="shellack-dev"):
...
def test_add_plugin_calls_install_plugin():
    ...
    say = _make_say()
    with patch.dict("os.environ", _owner_env()), patch.object(
        bot_unified.plugin_manager,
        "install_plugin",
        return_value={"ok": True, "stdout": "Installed"},
...
def test_remove_plugin_calls_uninstall_plugin():
    ...
    with patch.dict("os.environ", _owner_env()), patch.object(
        bot_unified.plugin_manager,
        "uninstall_plugin",
        return_value={"ok": True, "stdout": "Removed"},
...
def test_add_mcp_calls_add_mcp():
    ...
    with patch.dict("os.environ", _owner_env()), patch.object(
        bot_unified.plugin_manager,
        "add_mcp",
        return_value={"ok": True, "stdout": "Added"},
    ) as mock_add, patch("bot_unified.get_channel_name", return_value="shellack-dev"):
...
def test_add_bot_plugin_calls_add_bot_plugin():
    ...
    with patch.dict("os.environ", _owner_env()), patch.object(
        bot_unified.plugin_manager,
        "add_bot_plugin",
        return_value={"ok": True, "name": "cool-ext", "module": mock_module},
    ) as mock_add, patch("bot_unified.get_channel_name", return_value="shellack-dev"):
...
def test_remove_bot_plugin_calls_remove_bot_plugin():
    ...
    say = _make_say()
    with patch.dict("os.environ", _owner_env()), patch.object(
        bot_unified.plugin_manager, "remove_bot_plugin", return_value={"ok": True}
    ) as mock_remove, patch(
        "bot_unified.get_channel_name", return_value="shellack-dev"
    ):
    ...
    mock_remove.assert_called_once_with(
        "cool-ext", registry=bot_unified._bot_extensions
    )
    say.assert_called_once()
...
def test_non_owner_blocked_from_mutating_plugin_commands(command):
    ...
    say = _make_say()
    with patch.dict("os.environ", {"OWNER_SLACK_USER_ID": _OWNER_ID}), patch(
        "bot_unified.get_channel_name", return_value="shellack-dev"
...
def test_owner_allowed_mutating_plugin_commands(command):
    ...
    with patch.dict("os.environ", {"OWNER_SLACK_USER_ID": _OWNER_ID}), patch(
        ...
        bot_unified.plugin_manager,
        "install_plugin",
        return_value={"ok": True, "stdout": "ok"},
    ), patch.object(
        bot_unified.plugin_manager,
        "uninstall_plugin",
        return_value={"ok": True, "stdout": "ok"},
    ), patch.object(
        bot_unified.plugin_manager, "add_mcp", return_value={"ok": True, "stdout": "ok"}
    ), patch.object(
        bot_unified.plugin_manager,
        "remove_mcp",
        return_value={"ok": True, "stdout": "ok"},
        ...
        bot_unified.plugin_manager,
        "add_bot_plugin",
        return_value={"ok": True, "name": "ext", "module": MagicMock()},
    ), patch.object(
        bot_unified.plugin_manager, "remove_bot_plugin", return_value={"ok": True}
    ):
...
def test_plugins_list_allowed_for_non_owner():
    ...
    with patch.dict("os.environ", {"OWNER_SLACK_USER_ID": _OWNER_ID}), patch(
        ...
        bot_unified.plugin_manager,
        "list_all",
        return_value={"plugins": [], "mcps": [], "bot_plugins": []},
    ):
...
def test_owner_env_unset_blocks_all_users(command):
    ...
    ephemeral_text = mock_app.client.chat_postEphemeral.call_args[1]["text"]
    assert (
        "not configured" in ephemeral_text.lower()
        or "disabled" in ephemeral_text.lower()
    )
